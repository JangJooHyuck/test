...
public class Emailcheck {
    ...
    public static void main(String[] args) {
        ...
        String email = scan.next(); // 입력받은 문자를 email에 저장
        String emailpattern = "[a-zA-Z0-9_.+-]+@[a-zA-Z]+\\.[a-zA-Z]+"; // 이메일 정규 표현식
        if (email.matches(emailpattern)) { // email 의 값이 emaliRegex 의 조건과 맞으면
            System.out.println("이메일의 형식이 정상적입니다.");
            System.out.println("당신의 이메일은 " + email + " 입니다.");
        } else { // 조건이 안맞으면
            System.out.println("이메일의 형식이 정상적이지 않습니다.");
